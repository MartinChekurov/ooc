...
static OOC_Error ooc_objectDestructor(void *self) { return OOC_ERROR_NONE; }
...
static char *ooc_objectToString(const void *self) {
  ...
  size_t len =
      snprintf(NULL, 0, "%s@%p", className ? className : "Object", self);
...
OOC_Error ooc_classNew(void *class, ...) {
  ...
  if (c->super) {
    ...
    if ((c->modifiers & OOC_MODIFIER_ABSTRACT) &&
        (c->modifiers & OOC_MODIFIER_FINAL)) {
      error = OOC_ERROR_INVALID_MODIFIER;
      goto end;
    }
    const size_t offset = offsetof(OOC_Class, ctor);
    memcpy((char *)c + offset, (char *)c->super + offset,
...
static const OOC_InterfaceImpl *
ooc_classFindInterfaceImpl(const OOC_Class *class,
                           const OOC_Class *targetIface) {
...
static bool ooc_classValidateInterfaces(const OOC_Class *class,
                                        const OOC_InterfaceImpl *impls,
                                        unsigned int count) {
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    size_t vtableSize =
        ooc_classSelfSize(impl->interfaceClass) - sizeof(OOC_Class);
    if (ooc_classVerifyMethods(class, impl->vtableOffset,
                               impl->vtableOffset + vtableSize) !=
        OOC_ERROR_NONE) {
      return false;
    }
  }
...
static bool ooc_classValidateParentInterfaces(const OOC_Class *c) {
  for (const OOC_Class *cur = c; cur; cur = cur->super) {
    ...
    for (unsigned int i = 0; i < cur->interfaceCount; i++) {
      ...
      size_t vtableSize =
          ooc_classSelfSize(impl->interfaceClass) - sizeof(OOC_Class);
      if (ooc_classVerifyMethods(c, impl->vtableOffset,
                                 impl->vtableOffset + vtableSize) !=
          OOC_ERROR_NONE) {
        return false;
      }
...
OOC_Error ooc_classSetInterface(void *class, const void *impls,
                                unsigned int count) {
  if (!class) {
    return OOC_ERROR_INVALID_ARGUMENT;
  }
...
const void *ooc_classGetInterfaceVtable(const void *class,
                                        const void *interfaceType) {
...
const void *ooc_getInterfaceVtable(const void *self,
                                   const void *interfaceType) {
